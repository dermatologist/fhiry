...
def test_fhirsearch():
    ...
    assert len(df[df['code.codingcodes'].astype('string') == "['A00.0']"]) == 1
    assert len(df[df['code.codingcodes'].astype('string') == "['A01.0']"]) == 1
    assert len(df[df['code.codingcodes'].astype('string') == "['A02.0']"]) == 1
    assert len(df[df['code.codingcodes'].astype('string') == "['A03.0']"]) == 1
    assert len(df[df['code.codingcodes'].astype('string') == "['A04.0']"]) == 1

    # There is no resource with code A05.0 in the FHIR search results
    assert len(df[df['code.codingcodes'].astype('string') == "['A05.0']"]) == 0
